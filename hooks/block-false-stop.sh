...
# The byte cap stops at a LINE boundary, never inside one. `head -c` cut mid-JSON, so a
# final entry larger than the cap arrived as a fragment, classified "partial", and was
# allowed — the guard fell silent on exactly the long-output turns where a mid-plan stall
# is most likely. awk emits whole lines until the budget is spent, so one oversized entry
# is still delivered intact while the scan stays bounded overall.
last=$({ cat "$transcript"; printf '\n'; } \
  | { tail -r 2>/dev/null || tac 2>/dev/null; } | head -n "$MAX_SCAN_LINES" \
  | awk -v cap="$MAX_SCAN_BYTES" '{ print; seen += length($0) + 1; if (seen >= cap) exit }' \
  | jq -R -r -n '
  first(
